...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if (strlen(line) >= LimitRequestLine)
            {
                error(400);
                continue;
            }
            
            // if there are no CRLF
            if (strstr(line, "\r\n") == NULL)
            {
                error(400);
                continue;
            ...
            // divides the line in three tokens separated by SP
            ...
            char* queryptr = strchrnul(request_target, '?');
            char query[strlen(queryptr)];
            ...
            // copy the absolute-path to a new array
            char* beginptr = &request_target[0];
            char absolute_path[queryptr - beginptr + 1];
            for (int i = 0; i < sizeof(absolute_path); i++)
            {
                absolute_path[i] = request_target[i];
            }
            absolute_path[sizeof(absolute_path) - 1] = '\0';
            
            // create path, copy the root and then concatenate absolute-path
